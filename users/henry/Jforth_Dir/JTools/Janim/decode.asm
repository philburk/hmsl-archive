...
\   XOR version is for AnimBrush support
...
\ A0 = IN
\ A3 = YTABLE
ASM DECODE_VKPLANE (  in out ytable --- , for anims )
	movem.l	a2/a3/d5,-(rp)  ; save registers 
	move.l	(dsp)+,a2
	adda.l  a4,a2              \ a2 = absaddr bitmap
	move.l	(dsp)+,a0
	adda.l  a4,a0              \ a0 = absaddr delta plane data
	move.l	d7,a3
	adda.l  a4,a3              \ a3 = absaddr of ytable data
    move.l  (a3),d2            \ d2 = linebytes
...
ASM DECODE_XORVKPLANE ( in out ytable --- , for animbrushes )
	movem.l	a2-a3/d5,-(rp)  ; save registers 
	move.l	(dsp)+,a2
	adda.l  a4,a2            \ a2 = absaddr bitmap
	move.l	(dsp)+,a0
...
4$:	clr.w	d1	  \ clear hi byte of op
	...
	bra	7$        \ go back to column loop
			  ...
			  \ here we decode a "vertical same run"
6$:	move.b	(a0)+,d1  \ fetch the count
	move.b	(a0)+,d3  \ fetch the value to XOR!!!!!!!!!!!!!!!
	move.w	d1,d5     \ and do what it takes to fall into a "tower"
	asr.w	#3,d5     \ d5 holds # of times to loop through tower
	and.w	#7,d1     \ d1 is the remainder
	add.w	d1,d1
...
5$:	and.b	#$7f,d1   \ setting up a tower as above....
	...
	add.w	d1,d1
        add.w   (dsp)+,d1
	neg.w	d1
	jmp	50(pc,d1.w)    \ offset is 8*6 +2 bytes for the uniq tower
9$:	move.b	(a0)+,d1
        eor.b   d1,(a1)
	adda.w	d2,a1
	move.b	(a0)+,d1
