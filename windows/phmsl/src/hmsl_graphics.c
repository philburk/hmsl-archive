...
void hostDrawText( uint32 addr, int32 count )
{
	...
	TextOut( gHMSLContext->hg_hDC,
		...
		gHMSLContext->hg_At.y - textSize.cy,
		(char*)addr,
		count );
...
{
	Rectangle( gHMSLContext->hg_hDC, x1, y1, x2, y2 );
...
using PALETTEINDEX for this color and select them to the DC.  SetTextColor
...
void hostSetColor( int32 color )
{
	...
	SetTextColor( gHMSLContext->hg_hDC, PALETTEINDEX( color ) );
...
{
	SetBkColor( gHMSLContext->hg_hDC, PALETTEINDEX( color ) );
	return;
}
